let studentFactory = (studentName, studentEvent) => {
  try {
    ...
    const studentObject = Object.create(null, {
      ...
      message: {
        value:
          studentEvent.type === "ForkEvent"
            ? "-"
            : `"${
                studentEvent.payload.commits[
                  studentEvent.payload.commits.length - 1
                ].message
              }"`
      },
      repoURL: {
    ...
    return {
      name: {
        ...
        githubHandle: "Error with Github Username"
      },
      githubHandle: "Error with Github Username",
...
let getDiffDays = lastPush => {
  let date = parseInt(
    (new Date(Date.now()) - lastPush) / (1000 * 60 * 60 * 24)
  );
